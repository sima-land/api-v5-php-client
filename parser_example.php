...
$client = new \SimaLand\API\Rest\Client([
    ...
    'baseUrl' => 'https://www.sima-land.ru/api/v5',
]);
...
$itemCategoryList = new \SimaLand\API\Entities\ItemCategoryList($client, ['logger' => $logger]);
$itemCategoryStorage = new \SimaLand\API\Parser\Json(['filename' => '/dev/null']);

// Связь товара с категорией.
$unitList = new \SimaLand\API\Entities\UnitList($client, ['logger' => $logger]);
$unitStorage = new \SimaLand\API\Parser\Json(['filename' => '/dev/null']);
...
$parser->addEntity($itemCategoryList, $itemCategoryStorage);
$parser->addEntity($unitList, $unitStorage);
